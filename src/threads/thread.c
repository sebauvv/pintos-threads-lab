...
// Flag para saber el inicio del thread
static bool flag_begin_thread = false;
...
static unsigned thread_ticks;   /* # of timer hticks since last yield. */
               ...
               thread_func *function, void *aux) 
{
  ...
  // Primer run de un hilo
  aux_prev_run = intr_disable ();
  ...
  intr_set_level (aux_prev_run);

  /* Add to run queue. */
  thread_unblock (t);
  //se produce un cambio de contexto si la prioridad del nuevo hilo es mayor que la del hilo actual
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered(&ready_list, &t->elem, sort_by_priority, NULL); // Antes: el hilo se insertaba en la lista y mandado atrás
...
thread_set_priority (int new_priority) 
{
  ...
  thread_current ()->starting_priority = new_priority;
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->time_for_wake_up = 0;
  list_push_back (&all_list, &t->allelem);
  // se inicializa la prioridad base del hilo
  t->starting_priority = priority;
  list_init (&t->locks_on_hold);
}
...
void despertarThread(struct thread *t, void *aux UNUSED)
{
    if (t->status == THREAD_BLOCKED && t->time_for_wake_up > 0)
    {
        t->time_for_wake_up--;
        if (t->time_for_wake_up <= 0)
        {
            thread_unblock(t);
        }
    }
...
// Comparacion entre 2 threads por prioridad
bool 
...
void
add_locked_thread_to_queue(struct lock* lock)
{
  list_push_back (&thread_current ()->locks_on_hold, &lock->elem);
}

void
remove_thread_with_locks(struct lock* lock)
{
  list_remove (&lock->elem);
}

void
assign_waiting_lock(struct lock* lock_or_NULL)
{
  thread_current ()->lock_on_wait = lock_or_NULL;
}
...
donate_priority (struct lock* lock)
{
  ...
  if (lock->holder->lock_on_wait != NULL &&
        lock->holder->lock_on_wait->holder != NULL)
      donate_priority (lock->holder->lock_on_wait);
}
...
reset_priority(void)
{
  ...
  int lock_max_priority = thread_current ()->starting_priority;
  /* find largest priority in holding locks. */
  for (lock_e = list_begin (&thread_current ()->locks_on_hold); 
        lock_e != list_end (&thread_current ()->locks_on_hold);
         lock_e = list_next (lock_e))
    {
      ...
      else
      {
        ...
        waiting_thread = list_entry (list_max (&lock_tmp->semaphore.waiters, sort_by_priority, NULL), struct thread, elem);
  ...
  ASSERT (lock_max_priority >= thread_current ()->starting_priority);
...
thread_update_recent_cpu(struct thread *t, void *aux UNUSED)
{
  t->recent_cpu = FP_ADD_MIX (
                  FP_DIV (FP_MULT (FP_MULT_MIX (load_avg, 2), t->recent_cpu),
                          FP_ADD_MIX (FP_MULT_MIX (load_avg, 2), 1)), t->nice);
...
thread_tick_one_second (void)
{
  ...
  load_avg = FP_DIV_MIX (FP_ADD (FP_MULT_MIX (load_avg, 59),FP_CONST (num_of_waiting_threads)), 60);
